...
class VGMPlayer
{
  ...
  private:
    ...
    void handleYm2612Write(uint8_t port)
    {
        ...
        Serial.print("YM2612 write: port=");
        Serial.print(static_cast<unsigned long>(port));
        Serial.print(", addr=0x");
        if (address < 0x10U) {
            Serial.print('0');
        }
        Serial.print(static_cast<unsigned long>(address), HEX);
        Serial.print(", data=0x");
        if (value < 0x10U) {
            Serial.print('0');
        }
        Serial.println(static_cast<unsigned long>(value), HEX);
